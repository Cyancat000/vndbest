...
function applyFilters(filters) {
  ...
  if (filters.selectedType !== undefined) selectedType.value = filters.selectedType
}
...
<template>
  ...
  <div class="page-container pb-24">
    <SearchBase
      ...
      :loading="isLoading"
      :filters="{ query, selectedLang, selectedType }"
      @search="handleSearch"
      @clear="handleClear"
        ...
        <div class="space-y-2 pb-2">
          ...
          <Transition name="panel">
            <div v-if="showAdvancedFilters" class="grid grid-cols-2 sm:grid-cols-3 gap-2 pt-1">
              ...
              >
                <template #prefix>
                  <Icon icon="lucide:building-2" class="h-3.5 w-3.5 text-neutral-400 dark:text-neutral-500" />
                </template>
              </BaseSelect>
            </div>
          </Transition>
          ...
          <div class="flex items-center justify-between gap-3">
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2">
                ...
                <span class="shrink-0 inline-flex items-center rounded-md bg-neutral-50 dark:bg-neutral-800 px-1.5 py-0.5 text-[9px] font-bold text-neutral-400 dark:text-neutral-500 border border-neutral-100 dark:border-neutral-700">
                  {{ t(`metadata.lang.${item.lang}`, item.lang) }}
                </span>
              </div>
              <p v-if="item.original" class="text-[10px] text-neutral-400 dark:text-neutral-500 truncate mt-0.5">
                {{ item.original }}
              </p>
            </div>
