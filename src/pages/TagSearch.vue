...
const { translateTagName, reverseLookupTagName, searchTagsByZh } = useTranslation()
...
function fetchI18nResults(keyword) {
  ...
  try {
    i18nResults.value = searchTagsByZh(keyword)
  } catch (err) {
    console.error('i18n 标签搜索失败:', err)
  } finally {
...
async function loadMore() {
  ...
  page.value++
  await fetchTags(query.value, false)
}
...
async function goToI18nResult(item) {
  ...
  try {
    const res = await searchTags(item.en, { results: 5 })
    if (res && res.results && res.results.length > 0) {
      // 精确匹配英文名
      const exact = res.results.find(r => r.name === item.en)
      const tag = exact || res.results[0]
      goToDetail(tag.id)
    }
  } catch (err) {
    console.error('查找标签 ID 失败:', err)
  } finally {
