      ...
      >
        ...
        <div class="relative z-10 w-full sm:w-[420px] max-h-[80vh] bg-white dark:bg-neutral-900 sm:rounded-2xl rounded-t-2xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom duration-200">
          ...
          <div class="flex items-center justify-between px-4 pt-4 pb-2">
            <h3 class="text-base font-bold text-neutral-900 dark:text-neutral-100">选择特征</h3>
            <button
              @click="handleClose"
          ...
          <div v-if="localSelected.length > 0" class="px-4 pb-2">
            <div class="flex flex-wrap gap-1.5">
              <span
                ...
                :key="trait.id"
                class="inline-flex items-center gap-1 pl-2 pr-1 py-0.5 rounded-md border text-[11px] font-medium cursor-pointer transition-all hover:shadow-sm bg-violet-50 text-violet-600 border-violet-200 dark:bg-violet-900/30 dark:text-violet-400 dark:border-violet-800/50"
              ...
              <div class="space-y-1">
                <button
                  ...
                  :class="isSelected(trait)
                    ? 'border-violet-400 bg-violet-50 ring-1 ring-violet-400/20 dark:border-violet-400 dark:bg-violet-900/30 dark:ring-violet-400/20'
                ...
                >
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2">
                      <span class="font-medium text-sm text-neutral-900 dark:text-neutral-100 truncate">{{ translateTraitName(trait.name) }}</span>
                      <span class="shrink-0 inline-flex items-center rounded-md px-1 py-0.5 text-[9px] font-bold border uppercase bg-violet-50 text-violet-600 border-violet-200 dark:bg-violet-900/30 dark:text-violet-400 dark:border-violet-800/50">
                  ...
                  <div class="shrink-0 flex items-center gap-2">
                    <span class="text-[10px] text-neutral-400 dark:text-neutral-500 font-medium">{{ trait.char_count }} Chars</span>
                    <Icon
          ...
          <div class="flex items-center justify-between px-4 py-3 border-t border-neutral-100 dark:border-neutral-800 bg-neutral-50/80 dark:bg-neutral-800/80">
            <span class="text-xs text-neutral-500 dark:text-neutral-400">
              已选 <span class="font-bold text-neutral-900 dark:text-neutral-100">{{ localSelected.length }}</span> 个特征
            </span>
            <button
              @click="handleConfirm"
